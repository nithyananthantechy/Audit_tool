...
const SecurityHub: React.FC<Props> = ({ user }) => {
  const [records, setRecords] = useState<RecordType[]>([
      { id: '1', col1: 'Server Room B', col2: 'Tailgating Attempt', col3: 'Medium', status: 'Pending', date: '2026-07-05' },
      { id: '2', col1: 'Main Entrance', col2: 'Badge Reader Failure', col3: 'High', status: 'Completed', date: '2026-07-03' }
    ]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 relative">
      <div className="flex flex-col md:flex-row items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-white tracking-tight">Security <span className="text-blue-500">Hub</span></h1>
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mt-2">Physical Access & Incident Log</p>
        </div>
        <button 
      ...
      <div className="bg-white/[0.03] backdrop-blur-2xl rounded-[40px] border border-white/[0.08] p-8 min-h-[400px]">
        <div className="flex items-center justify-between mb-8">
          <h2 className="text-xl font-bold text-white">Physical Access & Incident Log Records</h2>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5">
                <th className="py-4 px-4 text-[10px] uppercase tracking-wider font-bold text-slate-400">Incident Area</th>
                <th className="py-4 px-4 text-[10px] uppercase tracking-wider font-bold text-slate-400">Incident Type</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-8 w-full max-w-md shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleAddEntry} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">Incident Area</label>
                <input 
                  type="text" required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">Incident Type</label>
                <input 
                  type="text" required
